        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                $("#buscar").click(function (e) {
                    e.preventDefault();
                    $.ajax({
                        url: "/<?php echo $_SESSION['RELATIVE_PATH'] ?>/IncidentesSW/ajax/buscarIncidente.php",
                        type: "POST",
                        data: "estado=" + $("#estado").val() + "&desde=" + $("#desde").val()
                                + "&hasta=" + $("#hasta").val() + "&sala=" + $("#sala").val()
                                + "&si=" + $("#si").val() + "&prioridad=" + $("#prioridad").val(),
                        success: function (opciones) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <select id="estado">
                                        ...
                                        <?php
                                        $consultaEstado = "SELECT E.nombre_estado AS nombre,"
                                                . " E.id_estado AS id FROM estado E";
                                ...
                                <td>
                                    <label for="labelprioridad">Prioridad:</label>
                                </td>
                                <td colspan="3">
                                    <select id="prioridad" name="prioridad">
                                        <option>Todos</option>
                                        <?php
                                        $consultaPrioridad = "SELECT P.nombre AS nombre, P.idPrioridad AS id
                                            FROM prioridad P";
                                        $resultadoPrioridad = mysql_query($consultaPrioridad);
                                        if (mysql_errno() == 0) {
